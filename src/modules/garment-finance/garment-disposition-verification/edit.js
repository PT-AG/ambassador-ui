...
export class Edit {
  ...
  async activate(params) {
    ...
    if (this.selectedExpedition) {
      ...
      let dispositionNote =
        await this.purchasingService.getDispositionNoteByIdVerifiedCalculated(
          this.selectedExpedition.DispositionNoteId,
        );
  ...
  async selectedExpeditionChanged(newValue, oldValue) {
    ...
    if (newValue) {
      var dispositionNote = await this.purchasingService.getDispositionNoteById(
        newValue.DispositionNoteId,
      );
  ...
  saveCallback(event) {
    ...
    if (!this.selectedExpedition) {
      ...
      this.dialog.show(CreateSubmit, this.submitContext).then((response) => {
        if (!response.wasCancelled) {
          if (response.output.context == "Cashier") {
            this.service
              .sendToCashier(this.selectedExpedition.Id)
              .then((result) => {
            ...
            this.service
              .sendToAccounting(this.selectedExpedition.Id)
              .then((result) => {
                alert("Data berhasil dibuat");
                const encoded = Base64Helper.encode(this.selectedExpedition.Id);
                this.router.navigateToRoute("view", { id: encoded });
      ...
      this.dialog.show(CreateSubmit, this.submitContext).then((response) => {
        ...
        if (!response.wasCancelled) {
          if (response.output.context == "Not Verified") {
            ...
            this.service
              .sendToPurchasingRejected(this.selectedExpedition.Id, remark)
              .then((result) => {
                alert("Data berhasil dibuat");
                const encoded = Base64Helper.encode(this.selectedExpedition.Id);
                this.router.navigateToRoute("view", { id: encoded });
