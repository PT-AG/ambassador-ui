...
export class items {
    ...
    async activate(context) {
        ...
        if (this.data.Id && !this.isFabric) {
            var stock = await this.service.searchStock({ size: 1, filter: JSON.stringify({ ProductId: this.data.Product.Id, UomId: this.data.Uom.Id, UnitId: this.data.Unit.Id, ReferenceType: "AVAL_BAHAN_PENOLONG" }) });
            if (!this.error)
