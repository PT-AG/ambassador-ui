...
export class Edit {
  ...
  async activate(params) {
    ...
    if (this.data && this.data.BookingOrderId) {
      this.booking = {};
      this.newBookingItem = [];
      var bookingData = await this.service.getBookingById(
        this.data.BookingOrderId,
      );
      if (
        moment(this.data.BookingDate).format("DD MMM YYYY") !==
      ...
      if (
        moment(this.data.DeliveryDate).format("DD MMM YYYY") !==
        moment(bookingData.DeliveryDate).format("DD MMM YYYY")
      )
      ...
      for (var detail of this.data.booking) {
        var bookingDetail = bookingData.Items.find(
          (item) => item.Id === detail.Id,
        );
        if (bookingDetail) {
          if (bookingDetail.ComodityId !== detail.ComodityId) {
            detail["bookingMasterPlanComodity"] = {
              name: bookingDetail.ComodityName,
              code: bookingDetail.ComodityCode,
            };
          ...
          if (
            bookingDetail.DeliveryDate &&
            detail.DeliveryDate &&
            moment(bookingDetail.DeliveryDate).format("DD MMM YYYY") !==
              moment(detail.DeliveryDate).format("DD MMM YYYY")
          )
            detail["bookingDeliveryDate"] = moment(
              bookingDetail.DeliveryDate,
            ).format("DD MMM YYYY"); //`${(new Date(bookingDetail.deliveryDate)).getDay()} - ${((new Date(bookingDetail.deliveryDate)).getMonth() + 1)} - ${(new Date(bookingDetail.deliveryDate)).getFullYear()}`;
  ...
  save(event) {
    ...
    if (this.booking) {
      ...
      if (this.booking.Remark) dataTemp.Remark = this.booking.Remark;
    }
