...
export class DataForm {
  ...
  async selectedCuttingInChanged(newValue, oldValue) {
    if (this.context.isCreate) {
      if (newValue) {
        ...
        Promise.resolve(
          this.service.getCuttingOut({
            filter: JSON.stringify({ EPOItemId: this.data.EPOItemId }),
          })
        ).then((result) => {
            ...
            for (var cuttingOutHeader of result.data) {
              for (var cuttingOutItem of cuttingOutHeader.Items) {
                for (var cuttingOutDetail of cuttingOutItem.Details) {
                  this.data.PlanPORemainingQuantity -=
                    cuttingOutDetail.CuttingOutQuantity;
                }
              }
            }
          }
  ...
  get cuttingInLoader() {
    return (keyword) => {
      ...
      return this.service.getCuttingInByRO(infoCuttingIn).then((result) => {
        var infoEPO = {
          keyword: keyword,
