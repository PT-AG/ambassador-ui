...
export class View {
  ...
  async activate(params) {
    ...
    switch (this.data.status) {
      ...
      case "REVISED_MD":
        this.alertInfo =
          "<strong>Alasan Revisi oleh Md:</strong> " +
          (this.data.statusActivities.slice(-1)[0] || {}).remark;
        break;
      case "REVISED_TO_MD":
        this.alertInfo =
      ...
      case "REVISED_SHIPPING":
        this.alertInfo =
          "<strong>Alasan Revisi oleh Shipping:</strong> " +
          (this.data.statusActivities.slice(-1)[0] || {}).remark;
        break;
      case "CANCELED":
        this.alertInfo =
          "<strong>Alasan Cancel:</strong> " +
          (this.data.statusActivities.slice(-1)[0] || {}).remark;
        break;
      default:
        break;
  ...
  saveCallback() {
    ...
    var notTemplate= this.data.items.find(a=>a.detailRows==null);
    console.log(notTemplate);
    if (this.data.documentsFile.length === 0 && notTemplate) {
    ...
    if (confirm(this.formOptions.saveText + "?")) {
      switch (this.data.status) {
        case "DRAFT":
          this.service.postBooking(this.data.id).then((result) => {
            this.cancelCallback();
          });
          break;
        case "DRAFT_POSTED":
        case "DRAFT_REJECTED_MD":
        case "DRAFT_REJECTED_SHIPPING":
          this.service.unpostBooking(this.data.id).then((result) => {
            this.cancelCallback();
          });
          break;
        case "DRAFT_APPROVED_SHIPPING":
          this.service
            .postPackingList(this.data.id)
            .then((result) => {
              this.cancelCallback();
            })
            .catch((error) => {
              ...
              if (
                ...
                errorNotif +=
                  "Tab DETAIL MEASUREMENT ada kesalahan pengisian.\n";
              }
        ...
        case "REJECTED_SHIPPING_UNIT":
        case "REVISED_SHIPPING":
          this.service.unpostPackingList(this.data.id).then((result) => {
            this.cancelCallback();
          });
          break;
        default:
