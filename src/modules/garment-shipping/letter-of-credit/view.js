...
export class View {
  ...
  async activate(params) {
    ...
    const amendLC = await this.service.searchAmendLC({
      filter: JSON.stringify({ documentCreditNo: this.data.documentCreditNo }),
    });
    this.amendLCData = [];
    ...
    this.finalAmount =
      ...
      amendLC.data.reduce((acc, cur) => (acc += cur.amount), 0);

    const packingList = await this.service.searchPackingList({
      filter: JSON.stringify({
        lcNo: this.data.documentCreditNo,
        isUsed: true,
      }),
    });
    if (packingList.data.length > 0) {
      const invoice = await this.service.searchInvoice({
        filter: JSON.stringify({ lcNo: this.data.documentCreditNo }),
      });
      this.invoiceData = [];
      ...
      this.totalInvoice = invoice.data.reduce(
        (acc, cur) => (acc += cur.amountToBePaid),
        0,
      );
