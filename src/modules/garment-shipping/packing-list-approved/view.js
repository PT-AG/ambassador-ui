...
export class View {
  ...
  deleteCallback(event) {
    this.dialogService
      .open({ viewModel: Dialog, model: { title: "Alasan Cancel" } })
      .then((response) => {
        if (!response.wasCancelled) {
          this.service
            .cancel({ id: this.data.id, reason: response.output })
            .then((result) => {
  ...
  revisiUnit(event) {
    this.dialogService
      .open({ viewModel: Dialog, model: { title: "Alasan Revisi" } })
      .then((response) => {
        if (!response.wasCancelled) {
          this.service
            .revise({ id: this.data.id, reason: response.output })
            .then((result) => {
  ...
  revisiMD(event) {
    this.dialogService
      ...
      .then((response) => {
        if (!response.wasCancelled) {
          this.service
            .reviseToMD({ id: this.data.id, reason: response.output })
            .then((result) => {
              alert("Packing List berhasil di-Revisi");
              this.cancelCallback();
            })
