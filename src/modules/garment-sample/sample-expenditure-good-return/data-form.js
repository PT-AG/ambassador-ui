...
import { Service,SalesService,PurchasingService, CoreService } from "./service";

const UnitLoader = require('../../../loader/garment-units-loader');

@inject(Service,SalesService,PurchasingService, CoreService)
export class DataForm {
    ...
    constructor(service,salesService,purchasingService, coreService) {
    ...
    get roLoader() {
        return (keyword) => {
            var info = {
              keyword: keyword,
              filter: JSON.stringify({UnitId: this.data.Unit.Id, "ExpenditureType.Contains(\"ARSIP\")": true})
            };
   ...
   async selectedDOChanged(newValue, oldValue){
    ...
    async selectedEGChanged(newValue, oldValue){
        ...
        if(newValue) {
            ...
            Promise.resolve(this.service.getExpenditureGood({ filter: JSON.stringify({ ExpenditureGoodNo: this.data.ExpenditureNo, UnitId: this.data.Unit.Id, "ExpenditureType.Contains(\"ARSIP\")": true}) }))
                    .then(result => {
